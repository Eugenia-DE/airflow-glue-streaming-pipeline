...
if not folders:
    raise Exception(f"[ERROR] No processed_date= folders found under {INPUT_VALIDATED_STREAMS_PATH}")
...
def write_status(state, msg):
    try:
        ...
        print(f"[ERROR] Failed to write job status: {e}")
        raise
...
try:
    ...
    if 'Contents' not in resp or not any(obj['Size'] > 0 for obj in resp['Contents']):
        write_status("SUCCESS_NO_DATA", f"No input files found in {INPUT_PATH}")
        sys.exit(0)

    df = spark.read.parquet(INPUT_PATH)
    if df.rdd.isEmpty():
        write_status("SUCCESS_NO_DATA", f"Parquet files exist but are empty at {INPUT_PATH}")
        sys.exit(0)
    ...
    genre_metrics = genre_df.join(user_avg_df, "track_genre").withColumn(
        "metric_date", lit(EXEC_TIMESTAMP[:10]).cast(DateType())
    )
    genre_metrics.write.mode("overwrite").partitionBy("metric_date").parquet(
        f"{OUTPUT_METRICS_BASE_PATH}/daily_genre_metrics"
    )
    ...
    top_3.write.mode("overwrite").partitionBy("metric_date").parquet(
        f"{OUTPUT_METRICS_BASE_PATH}/top_songs_per_genre"
    )

    # Top 5 genres overall
    genre_rank_df = df.groupBy("track_genre").agg(
        count("track_id").alias("total_genre_listens")
    )
    ...
    top_5.write.mode("overwrite").partitionBy("metric_date").parquet(
        f"{OUTPUT_METRICS_BASE_PATH}/top_genres"
    )
